        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex flex-wrap items-center gap-3">
              ...
              <span
                ...
                className={`px-3 py-1 rounded-full text-sm font-medium cursor-pointer transition-colors ${
                  zona === "Tutti"
                    ...
                    : "bg-gray-100 text-gray-700 hover:bg-[#003366] hover:text-white"
                }`}
              >
                {zona}
              </span>
          ...
          <div className="flex items-center gap-2 ml-auto">
            <div className="flex items-center gap-1 border border-gray-200 rounded-lg px-3 py-1.5 bg-gray-50">
              ...
              {[1, 2, 3, 4, 5].map((s) => (
                <span key={s} className="text-[#C8A951] cursor-pointer text-sm hover:scale-110 transition-transform">★</span>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {ACCOMMODATIONS.map((acc) => (
              <div key={acc.id} className="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow flex flex-col overflow-hidden">
                <div className="h-40 bg-gradient-to-br from-[#003366] to-[#004080] flex items-center justify-center text-white text-5xl">
                  🏨
                </div>
                <div className="p-5 flex flex-col flex-1">
                  ...
                  <div className="flex items-center gap-0.5 mb-2">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <span key={i} className={`text-sm ${i < acc.stelle ? "text-[#C8A951]" : "text-gray-200"}`}>★</span>
                    ))}
                  </div>
                  ...
                  <div className="flex items-center justify-between mt-auto">
                    <div>
                      ...
                      <span className="text-lg font-bold text-[#003366] mx-1">€{acc.prezzo}</span>
                      <span className="text-xs text-gray-400">/notte</span>
                    </div>
                    <button className="bg-[#003366] hover:bg-[#004080] text-white text-xs font-semibold px-4 py-2 rounded-lg transition-colors">
                      Disponibilità
                    </button>
                  </div>
                </div>
        ...
        <section>
          ...
          <div className="flex flex-wrap gap-3">
            {[
              ...
              <span
                key={partner.nome}
                className={`border-2 rounded-lg px-5 py-2.5 text-sm font-bold tracking-wide ${partner.colore}`}
              >
                {partner.nome}
              </span>
            ))}
